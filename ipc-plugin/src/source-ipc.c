...
typedef SCPacket Packet;

static int32_t suricata_set_packet_data(
        Packet * packet, //packet pointer
        uint8_t * data, //packet data
        uint32_t len,  //packet data length
        uint32_t linktype, //linktype
        uint32_t ts_sec, //ts_sec
        uint32_t ts_usec, //ts_usec
        OnExtPacketRelease release, //release function
        uint8_t * user_data //user data
) {
    return PacketSetDataWithRelease(packet, data, len, linktype, ts_sec, ts_usec, release, user_data);
}

SCError suricata_log_message(const SCLogLevel log_level, const char *file,
                             const unsigned int line, const char *function,
                             const SCError error_code, const char *message) {
    SCLogMessage(log_level, file, lin, function, error_code, message)
}

/**
 * Pcap File Functionality
 */
void TmModuleReceiveIpcPluginRegister (void)
{
...
void TmModuleDecodeIpcPluginRegister (void)
{
...
TmEcode ReceiveIpcLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    while (1) {
        ...
        SCLogInfo("Received %" PRIu64 " packets from ipc", packets_received);
